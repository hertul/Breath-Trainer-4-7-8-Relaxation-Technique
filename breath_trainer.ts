# breath_trainer.ts
/**
 * 🌬️ Breath Trainer – 4-7-8 Relaxation Technique (TypeScript Edition)
 * Fully typed, advanced: guided breathing, progress bars, stats, custom timing, sound
 */
...
class StatsManager {
    ...
    public totalTimeSeconds: number = 0;
    public sessions: Session[] = [];
    public customTiming: Timing = { inhale: CONFIG.defaultInhale, hold: CONFIG.defaultHold, exhale: CONFIG.defaultExhale };

    constructor() {
        const dir = CONFIG.dataDir;
        if (!fs.existsSync(dir)) fs.mkdirSync(dir, { recursive: true });
        this.filePath = path.join(dir, CONFIG.dataFile);
        this._load();
    }
    ...
    private _load(): void {
        if (fs.existsSync(this.filePath)) {
            try {
                const data: Stats = JSON.parse(fs.readFileSync(this.filePath, 'utf8'));
    ...
    recordSession(cycles: number, duration: number): void {
        this.totalSessions++;
    ...
    setTiming(inhale: number, hold: number, exhale: number): void {
...
class BreathTrainer {
    ...
    private async _askInt(prompt: string): Promise<number | null> {
        const ans = await this._ask(prompt);
        const num = parseInt(ans.trim());
        return isNaN(num) ? null : num;
    }

    private async _showStage(stage: string, duration: number, color: string, emoji: string): Promise<void> {
    ...
    private async _breathingCycle(): Promise<number> {
    ...
    async runSession(cycles: number = 4): Promise<void> {
